...
class SynthesizerTrn(nn.Module):
  ...
  def __init__(self, 
    ...
    **kwargs):
    ...
    if self.h1h2:
        self.enc_h1h2 = CreakEncoder(input_size=1, model_embedding_size=1024)
        self.enc_h1h2_flow = CreakEncoder(input_size=1, model_embedding_size=192)
    if self.pitch:
        self.enc_pitch = CreakEncoder(input_size=1, model_embedding_size=1024)
        self.enc_pitch_flow = CreakEncoder(input_size=1, model_embedding_size=192)


  def forward(self, c, spec, g=None, mel=None, c_lengths=None, spec_lengths=None, creaks=None, cpps=None, h1h2s=None, pitches=None):
    if c_lengths == None:
...
#        z = z + self.enc_creak(creaks)[1]
  ...
  def infer(self, c, g=None, mel=None, c_lengths=None, creaks=None, cpps=None, h1h2s=None, pitches=None):
    if c_lengths == None:
      c_lengths = (torch.ones(c.size(0)) * c.size(-1)).to(c.device)
    if not self.use_spk:
    ...
    if self.h1h2 and h1h2s is not None:
        c = c + self.enc_h1h2(h1h2s)[0]
    if self.pitch and pitches is not None:
        c = c + self.enc_pitch(pitches)[0]
    g = g.unsqueeze(-1)
